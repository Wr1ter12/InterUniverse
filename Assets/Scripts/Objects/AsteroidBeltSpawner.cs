...
public class AsteroidBeltSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for(int i = 0; i < asteroidDensity; i++)
        {
            ...
            GameObject _asteroid = Instantiate(prefab, worldPosition, Quaternion.Euler(Random.Range(0,360), Random.Range(0,360), Random.Range(0,360)));
            _asteroid.AddComponent<AsteroidBelt>().SetupBeltObject(Random.Range(minOrbitSpeed, maxOrbitSpeed), Random.Range(minRotationSpeed, maxRotationSpeed), gameObject, rotatingClockwise);
